...
class NeuralNetwork(val numberOfInputs: Int, val numberOfOutputs: Int, val hiddenLayerConfiguration: IntArray) {
    ...
    init {
        ...
        networkSynapses = Array(layerSizes.size-1, // create a weight matrix for every layer of the neural network except the output
                { i -> randomMatrix(layerSizes[i]+1,layerSizes[i+1]) }) // the matrix size is equal to (layer size + a bias weight) X (layer size of the next layer)
    }
    ...
    fun feedforward(inputs: FloatArray): FloatArray {
        ...
        for (i in 0 until networkSynapses.size) {
            justALayer = matrixMultiply(addBiasColumn(justALayer), networkSynapses[i]) // get next layer by multiplying current layer with a bias column by the synapses for that layer
    ...
    private val leakyReLuSlope = 0.01f
